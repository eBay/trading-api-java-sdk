...
public class GetCategoriesCall extends com.ebay.sdk.ApiCall {
    ...
    public static int getAllCategories(ApiContext apiContext,
        SiteCodeType siteID, int levelLimit, DetailLevelCodeType detailLevel,
        int groupSize, CategoryEventListener listener) throws Exception {
        ...
        for (length = 0; length <= tail; ) {
            if (rootCats[length].getCategoryLevel() == levelLimit && (
                rootCats[length].isLeafCategory() == null
                    || rootCats[length].isLeafCategory() == false)) {
                length++;
                continue;
            }
        ...
        listener.receivedCategories(siteID, topCategories,
            api.returnedCategoryVersion);
        ...
        for (int fromId = 0; fromId < length; ) {
            ...
            if (cats != null && cats.length > 0) {
                ...
                listener.receivedCategories(siteID, cats,
                    api.returnedCategoryVersion);
            }
    ...
    public CategoryType[] getCategories()
        ...
        GetCategoriesResponseType resp = (GetCategoriesResponseType) execute(
            req);

        this.returnedCategoryArray = (resp.getCategoryArray() == null ?
            null :
            resp.getCategoryArray().getCategory());
