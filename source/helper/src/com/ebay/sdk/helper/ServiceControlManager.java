...
public class ServiceControlManager {
    ...
    public ControlTagItem[] createPaymentServiceControl(SiteCodeType siteId,
        ...
        Hashtable map = PaymentServiceHelper.getInstance()
            .getPaymentServiceMap(siteId);
    ...
    public ControlTagItem[] createPaymentServiceControl(SiteCodeType site,
        JComboBox cbxControl, boolean addDefaultOption) {
        ControlTagItem[] ctrlServices = createPaymentServiceControl(site,
            addDefaultOption);
    ...
    public ControlTagItem[] createRegionServiceControl(SiteCodeType siteId,
        boolean addDefaultOption) {
        Hashtable map = RegionServiceHelper.getInstanace()
            .getRegionsMap(siteId);
    ...
    public ControlTagItem[] createShippingServiceControl(SiteCodeType siteId,
        String shippingType, boolean addDefaultOption) {
        Hashtable map = null;
        if (ShippingServiceHelper.FLAT_SHIPPING_METHODS.equals(shippingType)) {
            map = ShippingServiceHelper.getInstance()
                .getDomesticShippingServiceMap(siteId,
                    ShippingTypeCodeType.FLAT);
        } else if (ShippingServiceHelper.CALCULATED_SHIPPING_METHODS.equals(
            shippingType)) {
            map = ShippingServiceHelper.getInstance()
                .getDomesticShippingServiceMap(siteId,
                    ShippingTypeCodeType.CALCULATED);
        } else if (ShippingServiceHelper.INTL_FLATRATE_SHIPPING_SERVICES.equals(
            shippingType)) {
            map = ShippingServiceHelper.getInstance()
                .getInternalShippingServiceMap(siteId,
                    ShippingTypeCodeType.FLAT);
        } else {
            map = ShippingServiceHelper.getInstance()
                .getInternalShippingServiceMap(siteId,
    ...
    public ControlTagItem[] createShippingServiceControl(SiteCodeType site,
        String shippingType, JComboBox cbxControl, boolean addDefaultOption) {
        ControlTagItem[] ctrlServices = createShippingServiceControl(site,
            shippingType, addDefaultOption);
        ComboBoxModel dataModel = new DefaultComboBoxModel(ctrlServices);
        cbxControl.setModel(dataModel);
        cbxControl.setSelectedIndex(0);
        return ctrlServices;
    ...
    public BuyerPaymentMethodCodeType[] processUserPaymentMethods(
        ...
        if (cnt > 0) {
            ...
            if (cnt > 0) {
                ...
                for (int i = 0; i < cnt; i++) {
                    arrPaymentMethods[i] = (BuyerPaymentMethodCodeType) lstPayments.get(
                        i);
                }
            }
        }
    ...
    public ShippingServiceOptionsType processUserSelectedShippingService(
        CurrencyCodeType currencyId, JComboBox cbxShippingServiceSelected,
        ...
        if (selectedShippingService != null) {
            ...
            shippingServiceOptionsType.setShippingService(
            ...
            if (cost != null && cost.length() > 0) {
                ...
                shippingServiceOptionsType.setShippingServiceCost(
                    shippingServiceSelectedCost);
            }
        }
    ...
    public ShippingDetailsType processUserShippingDetails(
        ...
        if (cnt > 0) {
            ...
            for (int i = 0; i < cnt; i++) {
                sso[i] = ((ShippingServiceSelector) shippingServiceOptions.get(
                    i)).getSelectedShippingServiceOption();
            }
        ...
        if (cnt > 0) {
            ...
            for (int i = 0; i < cnt; i++) {
                sso[i] = ((InternationalShippingServiceSelector) intlShippingServiceOptions.get(
                    i)).getSelectedShippingServiceOption();
            }
    ...
    public void updateShippingServiceSelections(
        ArrayList shippingServiceOptions, JScrollPane spShippingServiceOptions,
        JButton btnRemoveShippingService, JButton btnAddShippingService) {
        String[] columnNames = { "Priority", "ShippingService", "Cost",
            "AdditionalCost" };
        int size = shippingServiceOptions.size();
        Object[][] dataTable = new Object[size][4];
        for (int i = 0; i < size; i++) {
            ShippingServiceSelector selector = (ShippingServiceSelector) shippingServiceOptions.get(
                i);
    ...
    public void updateIntlShippingServiceSelections(
        ...
        String[] columnNames = { "Priority", "ShippingService", "Cost",
            "AdditionalCost", "ShipToLocations" };
        int size = intlShippingServiceOptions.size();
        Object[][] dataTable = dataTable = new Object[size][5];
        for (int i = 0; i < size; i++) {
            InternationalShippingServiceSelector selector = (InternationalShippingServiceSelector) intlShippingServiceOptions.get(
                i);
            ...
            int cnt =
                arrShipToLocations != null ? arrShipToLocations.length : 0;
            if (cnt > 0) {
