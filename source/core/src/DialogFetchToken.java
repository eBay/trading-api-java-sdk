...
public class DialogFetchToken extends JDialog {
    ...
    public DialogFetchToken(Frame frame, ApiContext apiContext, String title,
        boolean modal) {
        ...
        try {
            ...
            if (this.apiContext == null)
                throw new Exception(
                    "DialogFetchToken: ApiContext is not specified.");
            ...
            if (signInUrl == null || signInUrl.length() == 0)
                throw new Exception(
                    "SignIn Url must be specified to call FetchToken.");
    ...
    private void jbInit() throws Exception {
        ...
        btnFetchToken.addActionListener(
            new DialogFetchToken_btnFetchToken_actionAdapter(this));
        ...
        btnGetSessionID.addActionListener(
            new DialogFetchToken_btnGetSessionID_actionAdapter(this));
        ...
        jLabel5.setText(
            "4) Click FetchToken API to retrieve the generated token text.");
        jPanel3.setBorder(BorderFactory.createEtchedBorder());
        jPanel9.setLayout(borderLayout11);
        jPanel14.setLayout(borderLayout12);
        jLabel6.setText(
            "3) Launch web browser with your secret and sign-in to generate the "
                + "token online.");
        btnLaunchWebBrowser.setText("Launch Web Browser");
        btnLaunchWebBrowser.addActionListener(
            new DialogFetchToken_btnLaunchWebBrowser_actionAdapter(this));
    ...
    private boolean validateApiAccount() {
        ...
        if (acc.getApplication().length() == 0)
            return false;
        if (acc.getCertificate().length() == 0)
            return false;
        return true;
    }
    ...
    void btnFetchToken_actionPerformed(ActionEvent e) {
        try {
            ...
            if (this.closeOnToken) {
                DialogAccount.showInfoMessage(this,
                    "The API token has been generated successfully!");
    ...
    private String getRuName() {
        ...
        return StringEscapeUtils.escapeJava(ruName);
    }
    ...
    void btnLaunchWebBrowser_actionPerformed(ActionEvent e) {
        try {
            String sessionID = StringEscapeUtils.escapeJava(this.txtSessionID.getText());
...
class DialogFetchToken_btnFetchToken_actionAdapter
...
class DialogFetchToken_btnGetSessionID_actionAdapter
    implements java.awt.event.ActionListener {
    DialogFetchToken adaptee;
...
class DialogFetchToken_btnLaunchWebBrowser_actionAdapter
    implements java.awt.event.ActionListener {
    DialogFetchToken adaptee;

    DialogFetchToken_btnLaunchWebBrowser_actionAdapter(
        DialogFetchToken adaptee) {
        this.adaptee = adaptee;
    }
